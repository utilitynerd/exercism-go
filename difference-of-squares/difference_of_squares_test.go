...
func TestSquareOfSumNaive(t *testing.T) {
	for _, test := range tests {
		if s := SquareOfSumNaive(test.n); s != test.sqOfSum {
			t.Fatalf("SquareOfSum(%d) = %d, want %d", test.n, s, test.sqOfSum)
		}
	}
}
...
func TestSumOfSquaresNaive(t *testing.T) {
	for _, test := range tests {
		if s := SumOfSquaresNaive(test.n); s != test.sumOfSq {
			t.Fatalf("SumOfSquares(%d) = %d, want %d", test.n, s, test.sumOfSq)
		}
	}
}
...
func BenchmarkSquareOfSumNaive(b *testing.B) {
	for i := 0; i < b.N; i++ {
		SquareOfSumNaive(100)
	}
}
